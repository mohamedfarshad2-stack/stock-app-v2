...
class HELOSDashboard extends Page
{
    ...
    public function getViewData(): array
    {
        ...
        $activeTeams = $hasMoneyRecords
            ? (int) MoneyRecord::query()->whereBetween('record_date', [$start, $end])->distinct('business_unit_id')->count('business_unit_id')
            : 0;

        $expectedToday = $hasDailyOps
            ? (float) DailyCODOperation::query()->whereDate('operation_date', $today)->sum('expected_profit')
            : 0.0;

        $expectedMonth = $hasDailyOps
            ? (float) DailyCODOperation::query()->whereBetween('operation_date', [$start, $end])->sum('expected_profit')
            : 0.0;

        $revenueToday = $hasDailyOps
            ? (float) DailyCODOperation::query()->whereDate('operation_date', $today)->sum(DB::raw('quantity * selling_price'))
            : 0.0;

        $revenueMonth = $hasDailyOps
            ? (float) DailyCODOperation::query()->whereBetween('operation_date', [$start, $end])->sum(DB::raw('quantity * selling_price'))
            : 0.0;

        $topProducts = $hasDailyOps && $hasProducts ? DailyCODOperation::query()
        ...
        $hasReturnLossEstimate = $hasProducts && Schema::hasColumn('products', 'return_loss_estimate');

        $highRiskProducts = $hasProducts ? Product::query()
            ->select([
                'name',
                DB::raw($hasReturnLossEstimate
        ...
        $businessUnitSummary = $hasDailyOps && Schema::hasTable('business_units') ? DailyCODOperation::query()
