        ...
        return [
            ...
            Action::make('import_sku_costs')
                ...
                ->action(function (array $data): void {
                    ...
                    if (count($rows) <= 1) {
                        Notification::make()->title('Excel has no data rows.')->danger()->send();
                        return;
                    }

                    $buMap = BusinessUnit::query()->pluck('id', 'name')->mapWithKeys(fn ($id, $name) => [mb_strtolower(trim((string) $name)) => $id]);
                    $imported = 0;
                    $errors = [];
                    ...
                    foreach (array_slice($rows, 1) as $i => $row) {
                        ...
                        $buId = $buMap[mb_strtolower(trim((string) ($row[0] ?? '')))] ?? null;
                        $strap = (float) ($row[5] ?? 0);
                        ...
                        $manufacturing = $strap + $stitch + $stitchEmp + $upper + $middle + $bottom + $glue;
                        ...
                        Product::updateOrCreate(
                            ...
                            [
                                ...
                                'is_active' => in_array(strtolower((string) ($row[19] ?? '1')), ['1', 'true', 'yes'], true),
                    ...
                    Notification::make()
                        ...
                        ->body($errors === [] ? 'Completed successfully.' : implode("\n", array_slice($errors, 0, 10)))
                        ->success($errors === [])
                        ->danger($errors !== [])
                        ->send();
                }),
